...
function getSteps() {
  return ['Choose targeted muscle', 'Choose your preffered exercise!'];
};
...
export default function UserDataPage() {
  ...
  function getMyFirstMuscleBuildingExercise_(preference,listExercise) {
    const requestOptions = {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const getMyFirstMuscleBuildingExercise=(username,muscle,preferedExerciseId,listExercise)=> {
    if (username!=''){
      ...
      fetch(`/AGR/FirstRecommend?username=${username}&exercise_id=${preferedExerciseId}&mode=2&fitness=Beginner&muscle=${muscle}`)
